...
AGENT_CHAT_ROOT = RUNTIME_ROOT / "agent_chat"
AGENT_CHAT_DB = AGENT_CHAT_ROOT / "agent_chat.sqlite3"
PATROL_ROOT = RUNTIME_ROOT / "patrol"
PATROL_DB = PATROL_ROOT / "patrol.sqlite3"
PATROL_UPLOAD_ROOT = PATROL_ROOT / "uploads"
LOCAL_GEOCODER_ROOT = RUNTIME_ROOT / "local_geocoder"
LOCAL_GEOCODER_DB = LOCAL_GEOCODER_ROOT / "geocode_cache.sqlite3"
LOCAL_GEOCODER_URL = os.getenv("LOCAL_GEOCODER_URL", "").strip().rstrip("/")
LOCAL_GEOCODER_TIMEOUT_SECONDS = max(1.0, float(os.getenv("LOCAL_GEOCODER_TIMEOUT_SECONDS", "8")))
...
for folder in (
    ...
    AGENT_CHAT_ROOT, PATROL_ROOT, PATROL_UPLOAD_ROOT, LOCAL_GEOCODER_ROOT,
    MINERU_CACHE_ROOT,
):
